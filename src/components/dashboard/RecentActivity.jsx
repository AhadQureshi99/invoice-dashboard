...
const StatusBadge = ({ status, type }) => (
  <span className={`text-xs ${badgeClass[type] || badgeClass.saved}`}>{status}</span>
)
...
const RecentActivity = () => {
  ...
  useEffect(() => {
    listActivity({ limit: 10 }).then(setRows).catch(() => {})
  }, [])
    ...
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
      ...
      <div className="px-6 pb-5 overflow-x-auto">
        <table className="w-full min-w-[620px]">
          ...
          <tbody>
            {rows.length === 0 && (
              <tr><td colSpan={5} className="text-center py-8 text-xs text-gray-400">No activity yet</td></tr>
            )}
            {rows.map((row) => (
              <tr key={row.id} className="border-b border-gray-50 last:border-0">
                <td className={tdClass}>{row.action}</td>
                <td className={`${tdClass} text-gray-800`}>{row.subject || '—'}</td>
                <td className={tdClass}><StatusBadge status={row.status} type={row.type} /></td>
                <td className={`${tdClass} text-gray-400`}>{fmt(row.created_at)}</td>
                <td className={`${tdClass} font-semibold text-gray-800`}>{row.user_id ? 'User' : 'System Auto'}</td>
              </tr>
            ))}
          </tbody>
        </table>
